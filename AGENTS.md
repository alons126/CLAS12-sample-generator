...
Use `src/workflows/support/environment.h` as the project-wide source of ANSI color definitions for maintained C++ in every current and future workflow. Other C++ files may select its semantic constants but must not define terminal escape sequences locally. Keep this header outside any one workflow because it is shared workflow infrastructure. Shell and Python launchers use the environment-variable palette in `src/launcher/presentation/` because they cannot include a C++ header.
