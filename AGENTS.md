...
Preserve the LUND particle-record contract shared by the archives: particles are active, momenta are in GeV/c, masses are in GeV/c², energy is in GeV and is calculated from the mass shell, vertices are in centimeters, and the remaining legacy status/parent fields are zero. Keep particle ordering stable: scattered electron first, followed by the channel nucleon for uniform ep/en or supported GST final-state particles in input order for GENIE.
