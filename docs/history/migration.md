...
The writer keeps the established whitespace and precision. Maintained uniform output deliberately differs from the archived per-file numbering: its event IDs now form one zero-based sequence across the complete run, matching the physical converter's input-wide numbering model. Particle masses come directly from the external target source, including its nonzero electron and six-decimal proton values. Production sampling uses the documented electron/charged-hadron p/1-p mixtures and uniform neutron momentum; `hadron-momentum=fixed` preserves the optional neutron-only 1 GeV/c study. Set matching channel, beam energy, target geometry, A/Z, file counts and seeds; choose the same file prefix when needed by downstream tools.
