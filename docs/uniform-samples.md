...
`lundfiles/lund-gen-monitoring/monitoring.root` contains per-PDG momentum, theta, phi, vertex, and two-dimensional distributions for every generated species. The legacy-named ROOT/PDF/PNG artifacts reproduce the archived layouts for `1e`, the tester, `epFD`, and `enFD`; new pion and CD modes rely on `lundfiles/lund-gen-monitoring/monitoring.root` because no archived histogram contract exists for them. Empty `mchipo/`, `reconhipo/`, and `rootfiles/` directories preserve the downstream layout. See [diagnostics](diagnostics.md) and [sampling equations](sampling-models.md).
