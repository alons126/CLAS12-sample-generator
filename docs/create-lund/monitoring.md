...
`UniformMonitoring` is implemented beside the generator in [`src/lund-generation/uniform-to-lund-converter/`](../../src/lund-generation/uniform-to-lund-converter). There is no separate shared monitoring layer because physical conversion does not consume it.
