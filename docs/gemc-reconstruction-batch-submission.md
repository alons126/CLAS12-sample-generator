...
The array size is the number of completed files, not the requested generation capacity. The default `JOB_NEVENTS` is the largest event count among the selected files. Physical LUND conversion uses `events-per-file` as both its rollover size and its remaining-input cutoff scale, keeping generation aligned with the intended per-task limit. Because the cutoff is evaluated after writing an accepted event and counts input entries rather than accepted events, a shorter final file can still remain in the array; that task reaches input EOF before `JOB_NEVENTS`. Confirm EOF behavior with the selected detector versions during server validation.
