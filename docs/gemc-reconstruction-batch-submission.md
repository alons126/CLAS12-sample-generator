...
Use a csh/tcsh login shell with the ifarm module command and reconstruction available. When `CLAS12TAGS_DIR` is empty, submission derives the shared clas12Tags parent from the inherited `GEMC_DATA_DIR` (or uses `/u/scigroup/cvmfs/geant4/almalinux9-gcc11/clas12Tags` when none is inherited), checks the parent and requested-version directory, and only then loads `gemc/<version>` in the Python child environment. Informational output produced by both the module unload and load operations remains visible in the submission transcript; generated Python environment code stays internal. The workflow verifies that the resulting `GEMC_DATA_DIR` is the requested version and that the resolved `gemc` executable is inside that directory. The report prints `SLURM_GEMC_EXECUTABLE`, which is the executable inherited by `sbatch`.
