...
Submission uses explicit shell settings: `NUM_OF_JOBS` selects numbered LUND files and `JOB_NEVENTS` supplies the shared per-task event limit to GEMC and reconstruction. A shorter final file stays in the same array and reaches input EOF; the limit is not an exact per-file count. The manifest remains creation provenance rather than a required submission input.
