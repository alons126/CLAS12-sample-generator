...
**With `--execute`, submission removes and recreates `OUTPATH/mchipo` and `OUTPATH/reconhipo` for both uniform and physical samples.** LUND inputs are preserved. Before calling `sbatch`, it writes `OUTPATH/reconhipo/slurm-submission-log.json` with every resolved parameter, the exact command, runtime Git repository/branch/commit/status/tracking information, and SHA-256 hashes for the GCARD, YAML, and worker payload. Preview is read-only and creates no log. Uniform monitoring is produced during LUND generation and does not use a simulation `rootfiles` directory. Checks reject unsafe output paths and symlinks before replacement. Optional farm-output cleanup deletes only files directly in the configured farm-output directory, once per invocation.
