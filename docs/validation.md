...
The setup parity test reads archived scripts into temporary fixtures and intercepts module and Slurm calls. Fifteen reference cases compare entire stdout byte-for-byte, with relocated paths and the unified payload location, feeding settings through the config resolver. Manifest-driven shell handoff is tested for both sources. Additional cases cover channel extensions, shorter final files with a shared event limit, multiple samples, output replacement and failures. No actual Slurm or detector jobs run during these tests. Input EOF behavior with the selected detector software remains part of server validation.
